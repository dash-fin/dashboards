...
SB_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVuZHltYnBkYXllaWRyb214YXliIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MzUzNTg1MCwiZXhwIjoyMDg5MTExODUwfQ.gvdEDz-4YPM8zvgR94EUv6JBpTNdR8u5G4EM3j3NSpI"
...
sb_get() {
  local url="${SB_URL}/rest/v1/$1"
  local params="$2"
  curl -s -G "$url" "${SB_HEADERS[@]}" --data-urlencode "$params" 2>/dev/null
}
# sb_get2 — multi-param query (ej: "select=col&limit=N&order=col.desc")
sb_get2() {
  local url="${SB_URL}/rest/v1/$1"
  shift
  local args=()
  for p in "$@"; do args+=("--data-urlencode" "$p"); done
  curl -s -G "$url" "${SB_HEADERS[@]}" "${args[@]}" 2>/dev/null
}
...
watchdog_usa() {
  ...
  now_epoch=$(date +%s)
  local fresh
  fresh=$(sb_get2 "mercado_usa" "select=updated_at" "limit=1" "order=updated_at.desc" | grep -oP '"updated_at":"\K[^"]+' | head -1 || true)
  if [[ -z "$fresh" ]]; then
  ...
  if (( now_epoch - fresh_epoch > max_age )); then
    log "WATCHDOG: mercado_usa sin actualizar desde $fresh — reiniciando tarea Windows"
    cmd.exe /c schtasks /run /tn "\\Dashboard USA" 2>/dev/null || true
    return 1
  fi
...
health_check() {
  ...
  resp=$(curl -s -o /dev/null -w "%{http_code}" -m 10 \
    ...
    -d '{"symbols":["SPY"]}' 2>/dev/null || echo "000")
  if [[ "$resp" != "200" ]]; then
...
earnings_watch() {
  ...
  local adrs
  adrs=$(sb_get2 "operaciones" "select=ticker" "activo=eq.true" "ticker_ars=like.*ADR*" 2>/dev/null | grep -oP '"ticker":"\K[^"]+' | sort -u || true)
  if [[ -z "$adrs" ]]; then
    # Fallback: buscar tickers de usa que estén en la watchlist
    adrs=$(sb_get "watchlist" "select=ticker" 2>/dev/null | grep -oP '"ticker":"\K[^"]+' | sort -u | head -20 || true)
  fi
  if [[ -z "$adrs" ]]; then
  ...
  for ticker in $adrs; do
    local earnings_json
    earnings_json=$(echo "$response" | grep -oP "\"${ticker}\":\{[^}]+\}" | head -1 || true)
    if [[ -z "$earnings_json" ]]; then continue; fi
    local earnings_date
    earnings_date=$(echo "$earnings_json" | grep -oP '"earningsDate":\[\[?\K[0-9]+' | head -1 || true)
    ...
    if (( earnings_epoch > hoy_epoch && earnings_epoch < dos_semanas )); then
      local earnings_str
      earnings_str=$(TZ=America/Argentina/Buenos_Aires date -d "@$earnings_epoch" '+%Y-%m-%d' 2>/dev/null || true)
...
hora=$(hour_art)
dia=$(date +%u)
if [[ "$dia" -gt 5 || 10#$hora -lt 810 || 10#$hora -gt 2030 ]]; then
