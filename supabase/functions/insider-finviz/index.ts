...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      await sb.from("cache_finviz_insider").select("cache_key").limit(1).catch(async () => {
        await fetch(sbUrl + "/rest/v1/rpc/", {
          method: "POST",
          headers: { "Content-Type": "application/json", "apikey": sbKey, "Authorization": "Bearer " + sbKey },
          body: JSON.stringify({ sql: `CREATE TABLE IF NOT EXISTS cache_finviz_insider (cache_key TEXT PRIMARY KEY, data JSONB, metadata JSONB, updated_at TIMESTAMPTZ DEFAULT NOW())` })
        }).catch(() => {});
      });
    } catch {
